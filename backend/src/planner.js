...
const PLANNER_PROMPT = [
  ...
  "For scheduling with invitees, prefer a single calendar task with action create_event or ensure_slot and include attendees as an array of email addresses so the calendar provider sends the invite.",
  "Use the separate email tool only when the user explicitly asks for a normal email instead of a calendar invite, or when you need a follow-up message that is not the invite itself.",
  ...
  "When the latest user message looks like a follow-up detail such as just a time, date, title, email address, or a brief confirmation, combine it with the recent conversation history before deciding whether planning is required.",
  "",
